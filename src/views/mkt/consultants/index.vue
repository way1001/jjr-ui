        ...
        <el-col :xs="24" :sm="24" :md="20" class="user__main">
          ...
          >
            ...
            <template slot="headimgUrl" slot-scope="scope">
              <img style="width: 100px" :src="scope.row.headimgUrl" />
            </template>
            <template slot="userQrcodeForm" slot-scope="scope">
              <MaterialList
                v-model="scope.row.userQrcode"
                type="image"
                :num="1"
                :width="300"
                :height="150"
            ...
            <template slot="userQrcode" slot-scope="scope">
              <img style="width: 100px" :src="scope.row.userQrcode" />
            </template>
          </avue-crud>
        </el-col>
      </el-row>
...
<script>
import { getPage, getObj, addObj, putObj, delObj } from "@/api/mkt/consultants";
import { tableOption } from "@/const/crud/mkt/consultants";
...
export default {
  ...
  computed: {
    ...
    permissionList() {
      return {
        addBtn: this.permissions["mkt:consultants:add"] ? true : false,
        delBtn: this.permissions["mkt:consultants:del"] ? true : false,
        editBtn: this.permissions["mkt:consultants:edit"] ? true : false,
        viewBtn: this.permissions["mkt:consultants:get"] ? true : false,
      };
    },
  },
  watch: {
  ...
  methods: {
      ...
      getPage(
        ...
        .then((response) => {
          ...
          tableData.forEach((item) => {
            ...
            item.userQrcode = [item.userQrcode];
          });
    ...
    handleUpdate: function (row, index, done, loading) {
      ...
      row.userQrcode = row.userQrcode[0];
      putObj(row)
        .then((response) => {
          this.$message({
            showClose: true,
